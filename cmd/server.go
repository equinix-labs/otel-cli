...
func serverCmd(config *otelcli.Config) *cobra.Command {
	cmd := cobra.Command{
		Use:   "server",
...
// stops or is killed.
func runServer(config otelcli.Config, cb otlpserver.Callback, stop otlpserver.Stopper) {
	// unlike the rest of otel-cli, server should default to localhost:4317
	if config.Endpoint == "" {
		config.Endpoint = defaultOtlpEndpoint
	}
	endpointURL, _ := otelcli.ParseEndpoint(config)
