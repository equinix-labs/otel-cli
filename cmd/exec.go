...
func execCmd(config *otelcli.Config) *cobra.Command {
	cmd := cobra.Command{
		Use:   "exec",
...
func doExec(cmd *cobra.Command, args []string) {
	...
	if config.IsRecording() {
		tp := otelcli.TraceparentFromSpan(span)
		...
		tp := otelcli.LoadTraceparent(config)
		if tp.Initialized {
			child.Env = append(child.Env, fmt.Sprintf("TRACEPARENT=%s", tp.Encode()))
		}
	}
